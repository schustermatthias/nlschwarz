...
void kernel_schwarz_dirichlet_1(const double *x, const long labelx, const double *y, const long labely,
                                const MeshType &mesh, double *kernel_val) {
...
void kernel_schwarz_dirichlet_2(const double *x, const long labelx, const double *y, const long labely,
                                const MeshType &mesh, double *kernel_val) {
    double min_label = min(labelx, labely);
    double max_label = max(labelx, labely);
...
void kernel_schwarz_dirichlet_3(const double *x, const long labelx, const double *y, const long labely,
                                const MeshType &mesh, double *kernel_val) {
    double min_label = min(labelx, labely);
    double max_label = max(labelx, labely);
...
void kernel_schwarz_dirichlet_4(const double *x, const long labelx, const double *y, const long labely,
                                const MeshType &mesh, double *kernel_val) {
    double min_label = min(labelx, labely);
    double max_label = max(labelx, labely);
...
void kernel_schwarz_dirichlet_5(const double *x, const long labelx, const double *y, const long labely,
                                const MeshType &mesh, double *kernel_val) {
    double min_label = min(labelx, labely);
    double max_label = max(labelx, labely);
...
void kernel_schwarz_neumann_1(const double *x, const long labelx, const double *y, const long labely,
                              const MeshType &mesh, double *kernel_val) {
    std::unordered_set<int> boundary_interaction = {22, 44, 24, 42};
    std::unordered_set<int> domain_interaction = {13, 31};
    std::unordered_set<int> domain_boundary_interaction = {12, 14, 21, 41};

    double d = 1. / (pow(mesh.sqdelta, 2));

    int key = labelx*10 + labely;
    if (boundary_interaction.count(key)) {
        *kernel_val = 0.0 * d;
    }  else {
        *kernel_val = 4.0 * d;
    }
...
void kernel_schwarz_neumann_2(const double *x, const long labelx, const double *y, const long labely,
                              const MeshType &mesh, double *kernel_val) {
    std::unordered_set<int> boundary_interaction = {22, 44, 24, 42};
    std::unordered_set<int> domain_interaction = {13, 31};
    std::unordered_set<int> domain_boundary_interaction = {32, 34, 23, 43};
...
void kernel_integrable_sym(const double * x, const long labelx, const double * y, const long labely,
    ...
    dist = vec_sqL2dist(x, y, 2);
    if (label == 11) {
        *kernel_val = 5.0 * 1. / (pow(mesh.sqdelta, 2)) * (1 - sqrt((dist/mesh.sqdelta)) );
    } else if (label==22){
