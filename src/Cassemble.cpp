...
void lookup_configuration(ConfigurationType & conf, int verbose=0){
    ...
    map<string, void (*)(const double * x, long labelx, const double * y, long labely,
            ...
            {"schwarz_dirichlet_1", kernel_schwarz_dirichlet_1},
            {"schwarz_dirichlet_2", kernel_schwarz_dirichlet_2},
            {"schwarz_dirichlet_3", kernel_schwarz_dirichlet_3},
            {"schwarz_dirichlet_4", kernel_schwarz_dirichlet_4},
            {"schwarz_dirichlet_5", kernel_schwarz_dirichlet_5},
            {"schwarz_neumann_1", kernel_schwarz_neumann_1},
            {"schwarz_neumann_2", kernel_schwarz_neumann_2},
    ...
    map<string, bool> lookup_singularKernels = {
            ...
            {"fractional", true},
            {"schwarz_dirichlet_1", true},
            {"schwarz_dirichlet_2", true},
            {"schwarz_dirichlet_3", true},
            {"schwarz_dirichlet_4", true},
            {"schwarz_dirichlet_5", true},
            {"singular", true},
            {"DEliaGlusa", true},
...
void par_assemble(const string compute, const string path_spAd, const string path_fd, const int K_Omega, const int K,
                  ...
                  int is_ShapeDerivative, int dg_solutions, const double *state, const double *adjoint
                  ) {
            ...
            */
            ...
            if (conf.is_ShapeDerivative) {
                if (conf.dg_solutions) {
                    ...
                    indices_solution = aDGdx;
                } else {
                    indices_solution = &mesh.Triangles(0, aTdx);
                }
                for (int i=0; i < mesh.dVertex; i++) {
                    state_nodal_values[i] = state[indices_solution[i]];
                    adjoint_nodal_values[i] = adjoint[indices_solution[i]];
                }
            }
